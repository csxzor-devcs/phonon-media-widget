...
class MediaWidget(tk.Tk):
    ...
    def set_playback_speed(self, speed_str):
        """Set playback speed (note: not all media players support this via Windows API)"""
        try:
            speed_value = float(speed_str.replace('x', ''))
            print(f"Playback speed set to {speed_value}x (if supported by media player)")
            # Note: Windows Media Transport Controls API doesn't directly support playback speed
            # This is a placeholder for future implementation or player-specific controls
        except:
            pass
    ...
    def restart_app(self):
        # Cleanly restart the application
        print("Restarting app...")
        python = sys.executable
        os.execl(python, python, *sys.argv)

    def pulse_btn(self, item):
        # Determine target color based on item type/tags
        tags = self.canvas.gettags(item)
        target_color = self.fg_color
        if "btn_shuffle" in tags or "btn_loop" in tags:
            target_color = self.artist_fg if hasattr(self, 'artist_fg') else "#909090"
             
        self.canvas.itemconfig(item, fill=self.sub_color)
        self.after(100, lambda: self.canvas.itemconfig(item, fill=target_color))
    ...
    # --- Ambilight Logic ---
    def create_glow_background(self, pil_img, width, height, radius):
        try:
            # 1. Resize to widget size (stretch is fine for ambient)
            # Resize small first for performance, then blur
            temp_w, temp_h = 100, int(100 * (height/width))
            img = pil_img.resize((temp_w, temp_h), Image.Resampling.LANCZOS)
            ...
            # 4. Resize to full size
            img = img.resize((int(width), int(height)), Image.Resampling.LANCZOS)
            
            # 5. Masking (Rounded Corners)
            mask = Image.new("L", (int(width), int(height)), 0)
            draw = ImageDraw.Draw(mask)
            draw.rounded_rectangle((0, 0, int(width), int(height)), radius=radius, fill=255)
